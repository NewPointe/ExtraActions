...
namespace org.newpointe.ExtraActions
{
    ...
    [MetricCategoriesField("Metric", "The Metric to add a Value for.", false, "", "", 0)]
    ...
    public class SaveMetricValue : ActionComponent
    {
        ...
        {
            var mList = data.MetricService.GetByGuids(MetricCategoriesFieldAttribute.GetValueAsGuidPairs(val.ResolveMergeFields(data.MergeFields)).Select(g => g.MetricGuid).ToList()).ToList();
        ...
        public override bool Execute(RockContext rockContext, WorkflowAction action, object entity, out List<string> errorMessages)
        {
            ...
            var metricList = GetFromAttribute(action, "Metric", "MetricAttribute", MetricListConverter, new MetricListConverterData { MergeFields = mergeFields, MetricService = new MetricService(rockContext) });
            var metricValueType = GetFromAttribute(action, "ValueType", "ValueTypeAttribute", EnumConverter<MetricValueType>, mergeFields);
            var metricValueDate = GetFromAttribute(action, "ValueDate", "ValueDateAttribute", DateConverter, mergeFields);
            var metricValueNote = GetFromAttribute(action, "ValueNote", "ValueNoteAttribute", StringConverter, mergeFields);
            ...
            var mValStr = GetAttributeValue(action, "MetricValue");
            if (Guid.TryParse(mValStr, out Guid mValGuid))
            {
                mValStr = action.GetWorklowAttributeValue(mValGuid);
            }
            var metricValueValue = mValStr.ResolveMergeFields(mergeFields).AsDecimalOrNull();
